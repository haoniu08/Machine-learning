 ...
 * which contains a number of floors, elevators, and requests.
 * It is responsible for providing a report of the building and elevator system.
 */
public class BuildingReport {
  ...
  /**
   * This toString method is used to generate a log of the building and elevator system.
   */
  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("Elevator System Status: ").append(systemStatus).append("\n");

    // Append elevator reports
    sb.append("Elevator Reports:\n");
    if (elevatorReports != null) {
      for (int i = 0; i < elevatorReports.length; i++) {
        ElevatorReport elevatorReport = elevatorReports[i];
        sb.append("Elevator ").append(i).append(" Report:")
            .append(" Current Floor: ").append(elevatorReport.getCurrentFloor())
            .append(" Direction: ").append(elevatorReport.getDirection())
            .append(" Door Closed: ").append(elevatorReport.isDoorClosed())
            .append(" Door Open Timer: ").append(elevatorReport.getDoorOpenTimer())
            .append(" End Wait Timer: ").append(elevatorReport.getEndWaitTimer())
            .append(" Out of Service: ").append(elevatorReport.isOutOfService())
            .append(" Taking Requests: ").append(elevatorReport.isTakingRequests())
            .append("\nFloor Requests: \n");
        boolean[] floorRequests = elevatorReport.getFloorRequests();
        for (int j = 0; j < floorRequests.length; j++) {
          if (floorRequests[j]) {
            sb.append("Floor ").append(j).append(": Requested\n");
          }
        }
      }
    } else {
      sb.append("Elevator Reports: No data available\n");
    }

    // Append up requests
    sb.append("Up Requests:\n");
    for (Request request : upRequests) {
      sb.append(request.getStartFloor()).append(" -> ").append(request.getEndFloor()).append("\n");
    }

    // Append down requests
    sb.append("Down Requests:\n");
